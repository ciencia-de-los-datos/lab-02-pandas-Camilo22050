...
def pregunta_06():
    ...
    """
    valores_unicos_mayusculas = sorted(tbl1['_c4'].str.upper().unique())
...
def pregunta_10():
    ...
    """
    tabla = tbl0.sort_values(by=["_c1","_c2"])
    tabla = tabla.groupby('_c1')['_c2'].apply(lambda x: ':'.join(x.astype(str))).reset_index()
...
def pregunta_12():
    ...
    # Agrupar los valores de _c5 por _c0 y aplicar join para concatenarlos
    tabla = tbl2.groupby('_c0')['_c5'].apply(lambda x: ','.join(sorted(map(str, x)))).reset_index()
...
def pregunta_13():
    ...
    """
    # Fusionar los DataFrames en base a la columna _c0
